...
async function main() {
    console.log(`🚀 Starting Bridged Deployment`);
    console.log(`📡 Bridge: ${RPC_URL} -> ${REAL_DEVNET}`);
    ...
    try {
        ...
        console.log(`\n📝 Updated .env with new local contract addresses and REAL devnet URL.`);
    }
    catch (error) {
        console.error('❌ Bridged Deployment failed:', error.message);
        if (error.data)
