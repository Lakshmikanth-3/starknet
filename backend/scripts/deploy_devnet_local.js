...
async function main() {
    console.log(`🚀 Starting Local Devnet Deployment (Fixed)`);
    ...
    try {
        // Get initial nonce from latest block
        ...
        const btcDeclare = await account.declare({ contract: btcSierra, casm: btcCasm }, { nonce });
        console.log(`✅ MockBTC Declared. Class Hash: ${btcDeclare.class_hash}`);
        await new Promise(resolve => setTimeout(resolve, 3000));
        nonce = (BigInt(nonce) + 1n).toString();
        ...
        const vaultDeclare = await account.declare({ contract: vaultSierra, casm: vaultCasm }, { nonce });
