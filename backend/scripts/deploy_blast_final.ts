...
const LOG_FILE = 'deploy_blast_final.log';
// Clear log
...
if (!PRIVATE_KEY) {
    log('❌ Private key missing!');
    process.exit(1);
}

log(`Using RPC (Fixed): ${RPC_URL}`);

const provider = new RpcProvider({ nodeUrl: RPC_URL });
const account = new Account(provider, ACCOUNT_ADDRESS, PRIVATE_KEY);

async function main() {
    log('🚀 Explicit V3 Deployment (BlastAPI) - FINAL ATTEMPT');
    ...
    try {
        ...
        try {
            ...
            log('⚠️ Class hash not found. Attempting Force Deploy (might fail if not declared).');
            // If not declared, we should declare. But deployContract can declare if we pass casm?
            // No, declare and deploy are separate usually.
            // But let's try force deploy first.
        }
