const { RpcProvider, Account, hash, Contract } = require('starknet');
const fs = require('fs');
...
async function main() {
    console.log("🚀 DEPLOYING TO SEPOLIA (Alchemy)\n");
    ...
    try {
        ...
        // Compute class hash
        const mockClassHash = hash.computeContractClassHash(mockBtcSierra);
        console.log(`📋 MockBTC Class Hash: ${mockClassHash}`);

        // Try to declare (will skip if already declared)
        console.log("   Checking if already declared...");
        try {
            await provider.getClassByHash(mockClassHash);
